...
<!--    
        ...
        @date:      16.07.2013
        @version:   1.0
        @desc:      print the note of a session
                    -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="CourseRef">
        <xsl:value-of select="//Root/Note/CourseRef"/>
    </xsl:variable>
    <xsl:template match="/">
    ...
    <xsl:template match="Note">
        <h3 class="heading">Note: <xsl:apply-templates select="//NoteInfos/SessionTopic/Topics"
            /></h3>
            ...
            <div class="doc_view">
                <div class="doc_view_header">
                    <dl class="dl-horizontal">
                        ...
                        <dt>Date</dt>
                        <dd>
                            <xsl:value-of select="//NoteInfos/LastEdit"/>
                        </dd>
                    </dl>
                </div>
                ...
                <div class="doc_view_footer clearfix">
                    <button title="Modifier la page"
                        onclick="javascript:window.location.href+='/modifier'"
                        class="btn btn-inverse faq_right "><i class="splashy-pencil"/> Edit</button>
                </div>
            </div>

            <!-- Button to go to the next and prev note in the cours -->
            <ul class="pager">
                <xsl:variable name="prevt">
                    <xsl:value-of select="//NoteInfos/PrevT"/>
                </xsl:variable>
                <xsl:variable name="nextt">
                    <xsl:value-of select="//NoteInfos/NextT"/>
                </xsl:variable>
                <xsl:if test="not(*[$prevt=''])">
                    <li class="previous">
                        <a href="{$xslt.base-url}me/mynotes/{$CourseRef}/{$prevt}"><i
                                class="icon-chevron-left"/> Previous</a>
                    </li>
                </xsl:if>
                <xsl:if test="not(*[$nextt=''])">
                    <li class="next">
                        <a href="{$xslt.base-url}me/mynotes/{$CourseRef}/{$nextt}">Next <i
    ...
    <!-- Generic template for usesfull print element -->
    ...
    <xsl:template match="Note" mode="path">
        <div id="jCrumbs" class="breadCrumb module">
            <ul>
                ...
                <li>
                    <a href="{$xslt.base-url}me/mynotes/{$CourseRef}">
                        <xsl:value-of select="//NoteInfos/CourseName"/>
                    </a>
                </li>
                <li>
                    <xsl:value-of select="//NoteInfos/SessionTopic"/>
