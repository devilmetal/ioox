...
<!-- 
        ...
        @date:      16.07.2013
        @version:   1.0
        @desc:      MyNotes present the list tried by period and by exist at less 1 note in the sbscribed cours
                    -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <site:view>
            ...
            <site:content>
                <xsl:choose>
                    <xsl:when test="/Root/Session/Id = '-1'">
                        <!-- NOT LOGGED IN  -->
                            ...
                            <ul>
                                <li>
                                    <a href="{$xslt.base-url}me/mynotes"><img
                                            src="{$xslt-ressource-url}/img/gCons-mini/addressbook.png"
                                            alt=""/>MyNotes</a>
                                </li>
                            </ul>
                        </div>
                        <!-- Retrive for each Period passed the data and apply the template -->
                        <div class="row-fluid">
    ...
    <!--    Print the Period head and for each child cours call the cours (only cours where I have note and I'm subscribed)-->
    <xsl:template match="Period">
        ...
        <div class="rowf-luid">
            <xsl:apply-templates select="Course[CourseId = //Root/Course/CourseId]"/>
        </div>


    </xsl:template>

    <!--    Template for print the box Course with corresponding link-->
    <xsl:template match="Course">
        ...
        <div class="span4">
            <div class="ourbox obc1 bb">
                ...
                <br/>
                <a href="{$xslt.base-url}me/mynotes/{$courseid}" class="btn btn-inverse btn-mini"><i
                        class="splashy-arrow_medium_upper_right"/> Show notes</a> 
                <a href="{$xslt.base-url}me/docs/{$courseid}" class="btn btn-inverse btn-mini"><i class="splashy-printer"></i> PDF</a>
            </div>
        </div>
    </xsl:template>

    <!-- Test purpose not delete -->
    <xsl:template match="Course" mode="past"> bho </xsl:template>
</xsl:stylesheet>
