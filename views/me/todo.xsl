...
<!-- 
        ...
        @date:      16.07.2013
        @version:   1.0
        @desc:      This page, to each logged user
                    offer a usesfull personal todolist
                    this data are integrated in the calendar
                    -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- The dotolist is composed of activity-->
    <xsl:template match="ToDoList">
        <xsl:apply-templates select="//Activity"/>
    </xsl:template>

    <!-- each activity is composed by groups of tasks -->
    ...
    <!-- For each Task exist some correlated data like priority and deadline -->
    <xsl:template match="Task">
        <xsl:variable name="prio">
            <xsl:value-of select="child::Priority"/>
        </xsl:variable>
        <blockquote class="prio{$prio}">
            <h3 class="priohead{$prio}">
                <xsl:value-of select="Title"/>
                <span class="oursmall priohead{$prio}"> (Prio: <xsl:value-of select="$prio"/>
                    <xsl:if test="count(Deadline) &gt; 0"> - deadline <xsl:value-of
    ...
    <!-- collection of usesfull template not commmented -->
    <xsl:template match="Time">
        <xsl:text> </xsl:text>
        <xsl:value-of select="format-time(.,'[H1]:[m1]')"/>
    </xsl:template>
